...
void HC_MD5_Update(MD5_CTX *ctx, const unsigned char *buf, size_t len) {
  uint32_t t;
...
void HC_MD5_Final(unsigned char digest[16], MD5_CTX *ctx) {
  unsigned count;
  unsigned char *p;
  uint32_t *a;
...
char *cs_md5(char buf[33], ...) {
  ...
  while ((p = va_arg(ap, const unsigned char *) ) != NULL) {
    ...
    HC_MD5_Update(&ctx, p, len);
  }
  va_end(ap);

  HC_MD5_Final(hash, &ctx);
